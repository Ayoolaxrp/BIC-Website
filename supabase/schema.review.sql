-- ============================================================================
-- Babcock Investors Club — Supabase schema (v3 · REVIEW DRAFT)
-- ============================================================================
-- DRAFT FOR REVIEW — DO NOT RUN until approved.
--
-- What this is:
--   A complete, idempotent (safe to re-run) schema that exactly matches what
--   the current frontend (bic-react) reads and writes. Derived from
--   supabase/schema.sql (v2) with the following REVIEW CHANGES:
--
--   1. FIX — articles.author
--      The Admin console's "Write Club Article" form inserts an `author`
--      column ("Babcock Investors Club Editorial Team") and ArticlePage /
--      Blog render it, but v2 had no such column — inserts would fail with
--      PostgREST once Supabase is connected. Added with a default so old rows
--      read cleanly.
--   2. ADD — rsvps.event_id (optional / future-proof)
--      A nullable FK to events(id) so RSVPs can later be tied to the exact
--      event. The frontend still stores `event_name` as text, so nothing
--      breaks either way. Safe to keep or drop.
--   3. ADD — targeted indexes for the exact queries the frontend runs:
--      member_applications(email), rsvps(email), events(is_upcoming),
--      articles(category), resources(sector), newsletter_posts(status).
--   4. NO CHANGE — every table, trigger, storage bucket, RLS policy and grant
--      from v2 is preserved verbatim in intent.
--
-- How to run (only after review + approval):
--   Supabase → SQL Editor → paste → Run. Then follow the runbook in the
--   README (auth provider, admin promotion, .env keys).
-- ============================================================================
...
--    REVIEW CHANGE: added `author` (the Admin club-article form sends it and
--    ArticlePage/Blog render it; v2 was missing the column).
-- ---------------------------------------------------------------------------
...
create table if not exists public.rsvps (
  ...
  email      text not null,
  event_name text,
  -- REVIEW CHANGE (optional): tie RSVPs to the exact event for future use.
  -- The frontend still writes event_name as text, so this is additive only.
...
-- ---------------------------------------------------------------------------
-- 7. INDEXES — REVIEW CHANGE: one per exact query the frontend runs.
--    (v2 relied on sequential scans; these keep lookups fast as data grows.)
-- ---------------------------------------------------------------------------
create index if not exists idx_member_applications_email on public.member_applications (email);  -- /member membership status lookup
create index if not exists idx_rsvps_email           on public.rsvps (email);                    -- /member RSVP history
create index if not exists idx_events_is_upcoming    on public.events (is_upcoming);             -- Events page upcoming/past filter
create index if not exists idx_articles_category     on public.articles (category);              -- Blog category filter
create index if not exists idx_resources_sector      on public.resources (sector);               -- Member sector resources
create index if not exists idx_newsletter_status     on public.newsletter_posts (status);        -- published posts query

-- ---------------------------------------------------------------------------
-- 8. STORAGE — buckets + policies for admin uploads
--    (images up to 5MB, resources up to 3.5MB)
-- ---------------------------------------------------------------------------
